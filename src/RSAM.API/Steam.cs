...
namespace RSAM.API
{
    public static class Steam
    {
        ...
        {
            var path = (string)Registry.GetValue(@"HKEY_CURRENT_USER\Software\Valve\Steam", "SteamPath", null);
            if (string.IsNullOrWhiteSpace(path) == false)
            {
                return path.Replace('/', Path.DirectorySeparatorChar);
            }

            path = (string)Registry.GetValue(@"HKEY_LOCAL_MACHINE\Software\Valve\Steam", "InstallPath", null);
            if (string.IsNullOrWhiteSpace(path) == false)
            {
                return path;
            }
        ...
        public static bool Load()
        {
            lock (_Sync)
            {
                ...
                string libraryName = Environment.Is64BitProcess ? "steamclient64.dll" : "steamclient.dll";
                string dependencyPath = Environment.Is64BitProcess ? path : Path.Combine(path, "bin");
                string libraryPath = Path.Combine(path, libraryName);
                ...
                IntPtr module;
                Native.SetDllDirectory(dependencyPath);
                try
                {
                    module = Native.LoadLibraryEx(libraryPath, IntPtr.Zero, Native.LoadWithAlteredSearchPath);
                }
                finally
                {
                ...
                try
                {
                    ...
                    if (_CallCreateInterface == null || _CallSteamBGetCallback == null || _CallSteamFreeLastCallback == null)
                    {
        ...
        // The selected Steam client module owns process-wide native state and is intentionally kept
        // loaded until process exit. RSAM creates several short-lived client sessions
        // in one process; unloading and loading the DLL between those sessions can
        // leave Steam IPC unable to create the next communication pipe.
    }
}
