...
namespace RSAM.Infrastructure.SteamSchema
{
    internal class KeyValue
    {
        ...
        public bool ReadAsBinary(Stream input)
        {
            try
            {
                return this.ReadAsBinary(input, 0) &&
                       (input.CanSeek == false || input.Position == input.Length);
            }
            catch (Exception)
            {
                this.Valid = false;
        ...
        {
            if (depth > MaxDepth)
                throw new FormatException("The binary KeyValue nesting is too deep.");
                ...
                switch (type)
                {
                    case KeyValueType.None:
                    {
                        if (current.ReadAsBinary(input, depth + 1) == false)
                            throw new FormatException("Invalid nested binary KeyValue object.");
                        break;
                    }
                    ...
                    default:
                        throw new FormatException($"Unsupported binary KeyValue type: {type}.");
                }
